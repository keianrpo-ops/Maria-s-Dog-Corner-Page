...
import { MessageCircle, Send, X, ShieldCheck, Heart, Sparkles, Phone, ShoppingCart, Dog } from 'lucide-react';
...
export const AIAssistant: React.FC<AIAssistantProps> = ({ setView }) => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { 
      role: ChatRole.MODEL, 
      text: "¡Hola! Soy Maria. 🐾 Me hace mucha ilusión conocer a nuevos perritos. Antes de nada, ¿cómo se llama tu peludo y qué edad tiene?" 
    }
  ]);
  ...
  const handleSend = async (customText?: string) => {
    ...
    const historyData = newMessages.map(msg => ({
      role: msg.role === ChatRole.USER ? 'user' as const : 'model' as const,
      parts: [{ text: msg.text }]
    }));

    const response = await generateDogAdvice(historyData);
  ...
  const quickActions = [
    { label: '🍪 Ver Snacks', view: PageView.SHOP, icon: <ShoppingCart size={14} /> },
    { label: '🐕 Servicios', view: PageView.SERVICES, icon: <Dog size={14} /> },
    { label: '📱 WhatsApp', action: () => window.open('https://wa.me/447594562006', '_blank'), icon: <Phone size={14} /> }
  ];

  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      {isOpen && (
        <div className="mb-4 w-[400px] max-w-[95vw] bg-white rounded-[2rem] shadow-2xl border border-gray-100 overflow-hidden flex flex-col h-[650px] animate-fade-in-up">
          {/* Header */}
          ...
          {/* Quick Action Bar (Restaurada) */}
          <div className="bg-gray-50 border-b border-gray-100 p-2 flex gap-2 overflow-x-auto no-scrollbar">
            {quickActions.map((action, i) => (
              <button
                key={i}
                onClick={() => action.view ? (setView(action.view), setIsOpen(false)) : action.action?.()}
                className="flex items-center gap-1.5 whitespace-nowrap bg-white border border-gray-200 text-brand-dark px-3 py-1.5 rounded-full text-xs font-bold hover:border-brand-teal hover:text-brand-teal transition-all shadow-sm"
              >
                {action.icon}
                {action.label}
              </button>
      ...
      <button
        ...
        className={`${isOpen ? 'scale-0' : 'scale-100'} transition-all duration-300 bg-brand-teal hover:bg-teal-700 text-white p-5 rounded-full shadow-2xl flex items-center justify-center group relative border-4 border-white`}
      >
